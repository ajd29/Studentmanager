...
public class MemoryManager {
     ...
     * @throws IOException
     */
    public void addFreeBlock(MemoryHandle freeBlock) throws IOException {
        int index = 0;
        for (int i = 0; i < freeList.size(); i++) {
     ...
     * @throws IOException
     */
    public void mergeFree() throws IOException {
        while (canMerge()) {
            mergeBlocks();
        }
    }
    ...
    public void mergeBlocks() throws IOException {
        for (int i = 0; i < freeList.size(); i++) {
            ...
            if ((i + 1) < freeList.size() && end == freeList.get(i + 1)
                .getPos()) {
                int length = freeList.get(i).getLength() + freeList.get(i + 1)
                    .getLength();
     ...
     * @throws IOException
     */
    public boolean isLastBlockFree() throws IOException {
        MemoryHandle lastFree = freeList.get(freeList.size() - 1);

        return (lastFree.getPos() + lastFree.getLength() == memFile.length());
     ...
     * @throws IOException
     */
    public void removeLastFree() throws IOException {
    ...
    public void printFreeList() {
        ...
        for (int i = 0; i < freeList.size(); i++) {
            result += "\nFree Block " + (i + 1) + " starts from Byte "
                + freeList.get(i).getPos() + " with length " + freeList.get(i)
                    .getLength();
        }
